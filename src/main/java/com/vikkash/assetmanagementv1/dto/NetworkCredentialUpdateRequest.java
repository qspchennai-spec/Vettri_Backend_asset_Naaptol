...
/**
 * Like NetworkCredentialCreateRequest, but `password` and `enablePassword`
 * are optional: if left blank, the existing encrypted value on the record
 * is preserved untouched. This avoids forcing an admin to know/re-enter a
 * device's current password just to update its location or notes.
 */
public class NetworkCredentialUpdateRequest {
    ...
    /** Optional — blank/null means "keep current password". */
    private String password;

    /** Optional — blank/null means "keep current enable password". */
    ...
    public NetworkCredentialUpdateRequest() {}
